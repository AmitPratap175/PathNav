...
def get_drivable_area_midpoint(da_seg_mask, ctr = 0):

    height, width = da_seg_mask.shape
    # print('Width: ',width)
    midpoints = []

    for row in range(int(0.4 * height),int((2.5/3.5)*height)):
        green_indices = np.where(da_seg_mask[row] == 1)[0]  

        if green_indices.size > 0:
            #find the continuous patches
            diffs = np.diff(green_indices)
            continuous_patches = np.split(green_indices, np.where(diffs != 1)[0] + 1)

            #find the largest continuous patch
            largest_patch = max(continuous_patches, key=len)
            # print(width//2 in largest_patch)
            if len(largest_patch) > 100 and width//2 in largest_patch:
                #calculate the midpoint of the largest patch
                midpoint = int(np.mean(largest_patch)) 
    ...
    try:
        if midpoints != []:
            ...
            x = midpoints[:, 0].reshape(-1, 1)  # Reshape for sklearn which expects 2D array for features
            y = midpoints[:, 1]

            # Step 2: Fit a polynomial curve using RANSAC
            degree = 1
            ransac = make_pipeline(PolynomialFeatures(degree), RANSACRegressor())
            ransac.fit(x, y)

            # Generate x values for plotting the polynomial curve
            x_curve = np.linspace(min(x), height, 500).reshape(-1, 1)
            y_curve = ransac.predict(x_curve)

            # np.savetxt(f'/rebhu/ComputerVisionTechniques/YOLOP/inference/output/images/deg_1np/heights_{ctr}.txt',y_curve)

            points = np.column_stack((y_curve, x_curve)).astype(np.int32)

            # np.savetxt(f'/rebhu/ComputerVisionTechniques/YOLOP/inference/output/description/midpoints/midpoints_{ctr}.txt', midpoints)

            return midpoints, points

        else:
            print('\n\n\n\n No midpoints detected!!!\n\n\n')
            return None, None
...
def show_seg_result(img, result, palette=None,is_demo=False):
    ...
    img = cv2.line(img.astype(
                        np.uint8), (img.shape[1]//2, img.shape[0]), (img.shape[1]//2, img.shape[0]//2), (0, 0, 255), 1)
    
    # Find drivable area midpoint
    midpoint_x, points = get_drivable_area_midpoint(result[0])  # Assuming result[0] is da_seg_mask

    if midpoint_x is not None:
        # Plot the midpoint on the image
        for row, midpoint in midpoint_x:
            if midpoint is not None:
                cv2.circle(img, (midpoint, row), 2, (0, 255, 255), -1)  # Yellow circle
        
        cv2.polylines(img, [points], isClosed=False, color=(255, 255, 255), thickness=2)

        # print(f'Points: {points[-1]}')
        # np.savetxt()
        msg = offset(img.shape[1]//2, points[-1][0])
        msg_angle = angle_offset(np.array([[img.shape[1]//2, img.shape[0]], [img.shape[1]//2, img.shape[0]//2]]),\
                                np.array([[points[-1][0], points[-1][1]],[points[0][0], points[0][1]]]))
        # print(msg_angle)

        final_msg = msg + '  Angle: ' + str(msg_angle)
        # print(final_msg)
        #write text
        ...
        (text_width, text_height), baseline = cv2.getTextSize(final_msg, font, font_scale, font_thickness)

        # Calculate the position to put the text in the center of the top part of the image
        x = (img.shape[1] - text_width) // 2
        y = text_height + baseline

        # Put the text on the image
        cv2.putText(img, final_msg, (x, y), font, font_scale, color, font_thickness)
    return img
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, auto=True):
        ...
        s = np.stack([letterbox(x, self.img_size, auto=self.auto)[0].shape for x in self.imgs], 0)  # shapes
    ...
    def __next__(self):
        ...
        img, _, pad = letterbox(img0[0], self.img_size, auto=self.rect and self.auto)
...
class SingleCamDistanceMeasure():
	...
	def __init__(self, object_list ="car" ):
...
def offset(shape, midpoint):
    
    offset = midpoint - shape
    if offset == 0:
        return "Centered"
    elif offset < 0:
        return f'Move Left. Offset: {offset}'
    else:
        return f'Move Right. Offset: {offset}'

def angle_offset(line1, line2):
    v1, v2 = np.array(line1[1]) - np.array(line1[0]), np.array(line2[1]) - np.array(line2[0])
    dot_product = np.dot(v1, v2)
    cross_product = np.cross(v1, v2)
    angle = np.degrees(np.arccos(dot_product / (np.linalg.norm(v1) * np.linalg.norm(v2))))
    return angle if cross_product >= 0 else -angle

def transform_pipeline_image(image,ctr):
    """
    ...
    dest_points = np.float32([
        [image_shape[0] / 5, 0],
        [image_shape[0] / 5, image_shape[1]],
        [4 * image_shape[0] / 5, image_shape[1]],
        [4 * image_shape[0] / 5, 0]
    ])
    ...
    warped = cv2.warpPerspective(image, transform_matrix, 
                (image_shape[0], image_shape[1]), flags=cv2.INTER_LINEAR)

    # gray = cv2.cvtColor(warped, cv2.COLOR_RGB2GRAY)
    # binary_image = cv2.threshold(gray, 10, 255, cv2.THRESH_BINARY)[1]

    # cv2.imwrite(f'/rebhu/ComputerVisionTechniques/YOLOP/inference/output/description/{ctr}.png',warped)
